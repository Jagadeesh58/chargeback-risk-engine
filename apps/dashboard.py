...
with tab1:
    ...
    if st.button("Score Dispute", type="primary"):
        ...
        try:
            ...
            colA.metric("Hybrid Risk", f"{result['win_probability']:.1%}")
            colB.metric("Calibrated", f"{result['calibrated_win_probability']:.1%}")
            colC.metric("Logistic Signal", f"{result['ml_win_probability']:.1%}")
            colD.markdown(f"### :{action_color}[{result['action']}]")
            colE.metric("Expected Value", f"Rs {result['expected_value']:,.2f}")
            st.caption(
                "Hybrid risk combines rules + Logistic Regression + HGB as an advisory signal. "
                "Evidence, economics, graph-risk safeguards and deterministic policy select the action."
            )
