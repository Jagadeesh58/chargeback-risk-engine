...
def main():
    ...
    lr = LogisticRegression(max_iter=1000, random_state=42).fit(
        X_train, train["would_win"].astype(int)
    )
    hgb = HistGradientBoostingClassifier(
        max_depth=4, learning_rate=0.06, max_iter=180, random_state=42
    ).fit(X_train, train["would_win"].astype(int))

    lr_p = lr.predict_proba(X_test)[:, 1]
    hgb_p = hgb.predict_proba(X_test)[:, 1]
    rule_p = pd.Series(
        [predict_win_probability(r.to_dict()) for _, r in test.iterrows()]
    ).to_numpy()
    total = sum(HYBRID_MODEL_WEIGHTS.values())
    hybrid_p = (
        HYBRID_MODEL_WEIGHTS["rules"] * rule_p
        + HYBRID_MODEL_WEIGHTS["logistic"] * lr_p
        + HYBRID_MODEL_WEIGHTS["tree"] * hgb_p
    ) / total
    ...
    results = {
        ...
        "models": {
            "rules": metrics(test["would_win"], rule_p),
            "logistic_regression": metrics(test["would_win"], lr_p),
            "hist_gradient_boosting": metrics(test["would_win"], hgb_p),
            "hybrid_risk": metrics(test["would_win"], hybrid_p),
        },
        "hybrid_weights": HYBRID_MODEL_WEIGHTS,
    }
    ARTIFACTS_DIR.mkdir(exist_ok=True)
    (ARTIFACTS_DIR / "temporal_evaluation.json").write_text(
