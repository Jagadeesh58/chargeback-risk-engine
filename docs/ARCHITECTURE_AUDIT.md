...
- There is no relationship graph or broader abuse context.
- ML is a comparison signal rather than a feature-rich predictive model.
- There is no explicit outcome-feedback table or temporal evaluation.
...
**Caveat:** in the live API and Streamlit deployments, `score_hybri()`
is called with no persisted `risk_graph`, so each request analyzes a
brand-new, empty graph — cross-dispute clustering only shows up in
`scripts/demo.py`, which explicitly seeds and reuses one `RiskGraph`
instance across calls. Wiring a process-level shared graph into
 `api.py` / `local_pipeline.py` is a natural next step, not yet done.
